...
Versione: v4.0 (STIL Integration)
Autore: ezrad & IA
Anno: 2026
"""
...
class RAWGGameImages:
    ...
    def search_game_c64_only(self, game_name):
        ...
        params = {
            'key': self.api_key,
            'search': game_name,
            'platforms': self.c64_platform_id,
            'page_size': 5,
            'ordering': '-rating'
        }
        ...
        try:
            ...
            debug_print(f"Errore ricerca RAWG per '{game_name}': {e}")
            return None
...
class IGDBGameImages:
    ...
    def search_game_c64_only(self, game_name):
        ...
        try:
            debug_print(f"Invio richiesta a IGDB per: '{game_name}'")
            ...
            if response.status_code == 401:
                ...
                debug_print(f"ERRORE IGDB {response.status_code}: {response.text}")
                return None

            response.raise_for_status()
            games = response.json()
            debug_print(f"IGDB: trovati {len(games)} giochi totali")
            ...
            if games:
                ...
                debug_print(f"IGDB: {len(c64_games)} giochi per C64")
                ...
                if c64_games:
                    debug_print(f"⚠️  Nessuna copertina, uso primo gioco C64: {c64_games[0].get('name', 'N/A')}")
                    return c64_games[0]
                else:
                    debug_print("Nessun gioco C64 trovato")
                    return None
            else:
                debug_print(f"IGDB: nessun gioco trovato per '{game_name}'")
                return None

        except requests.exceptions.Timeout:
            debug_print(f"Timeout nella richiesta IGDB per '{game_name}'")
            return None
        except Exception as e:
            debug_print(f"Errore ricerca IGDB per '{game_name}': {type(e).__name__} - {e}")
            return None
...
class SidTkPlayer:
    def __init__(self, master, config=None):
        ...
        # Nuova label per il sottotitolo STIL (subsong)
        ...
        # Nuova label per autore/copyright
        ...
        if not os.path.exists(self.images_dir):
            ...
            debug_print("Creo la directory 'images'...")
            os.makedirs(self.images_dir, exist_ok=True)

        # Carica automaticamente la playlist se esiste
        self.load_playlist_file()
        
        # Inizializza le label con lo stato iniziale
    ...
    def load_playlist_file(self):
        ...
        if not os.path.exists(playlist_path):
            ...
            debug_print(f"  ⚠ Playlist non trovata")
            return False
    ...
    def find_game_image(self, sid_path, sid_title):
        """Cerca un'immagine per il gioco in base al TITOLO SID."""
        ...
        for ext in image_extensions:
            ...
            if os.path.exists(image_path):
                debug_print(f"Immagine trovata localmente per titolo '{sid_title}': {image_path}")
                return image_path, "Local file (by SID title)"

            image_path = os.path.join(self.images_dir, safe_title + '_COVER' + ext)
            if os.path.exists(image_path):
                debug_print(f"Copertina trovata localmente per titolo '{sid_title}': {image_path}")
                return image_path, "Local file (cover by SID title)"

        if self.igdb_fetcher:
            debug_print(f"Provo IGDB per copertina C64 con titolo SID: '{sid_title}'")
        ...
        if self.rawg_fetcher:
            debug_print(f"Provo RAWG filtrato per C64 con titolo SID: '{sid_title}'")
        ...
        debug_print(f"Nessuna immagine trovata per il titolo SID: '{sid_title}'")
        return None, None
    ...
    def play_next_track(self):
        ...
        # === GESTIONE TITOLI ===
        # 1. Titolo principale: dall'header SID (o nome file) - è il titolo del GIOCO
        # 2. Sottotitolo: titolo STIL del subsong specifico (se disponibile)
        # 3. Autore e copyright
        
        header_title = self.get_sid_title(track_path)
        author = self.get_sid_author(track_path)
        released = self.get_sid_released(track_path)
        
        # Cerca titolo STIL per il subsong corrente
        stil_title = None
        if self.stil_reader.loaded:
            stil_title = self.stil_reader.get_title(track_path, subsong, fallback_to_filename=False)
        
        # Il titolo PRINCIPALE è sempre quello dell'header SID (o nome file)
        # Questo serve anche per la ricerca delle copertine
        main_title = header_title
        
        # Mostra il titolo principale (grande)
        self.label_title.config(text=main_title, fg=C64_PALETTE["LIGHT_GREEN"])
        
        # Mostra il titolo STIL del subsong (piccolo, stessa dimensione autore)
        if stil_title and stil_title != main_title:
            ...
            if stil_title is None and self.stil_reader.loaded:
                log_message(f"STIL non trovato per {os.path.basename(track_path)} (subsong {subsong})")
        
        # Mostra autore e copyright (piccolo, stessa dimensione STIL)
        ...
        # === Cerca l'immagine usando il titolo PRINCIPALE (quello del gioco) ===
        # Non usare il titolo STIL del subsong per la ricerca!
        image_path, image_source = self.find_game_image(track_path, main_title)
        if image_path:
            self.load_and_display_image(image_path, image_source)
        else:
            status_text = f"[No image found for: {main_title}]"
    ...
    def show_about(self):
        ...
        about_window.geometry("420x550")  # Ridotto da 520x600
        ...
        if banner_path:
            try:
                ...
                # Ridimensiona a max 320px larghezza (ridotto da 480)
                max_width = 320
        ...
        desc_label = tk.Label(about_window,
                             text="C64 SID Music Player\nwith STIL subsong titles & cover art",
        ...
        display_info = "Display: Game Title (main) + Subsong Title (small)"
        display_label = tk.Label(about_window, text=display_info,
                                font=(self.font_family, 8),
                                fg=C64_PALETTE["GREY"],
                                bg=C64_PALETTE["BLACK"])
        display_label.pack(pady=(5, 5))
        ...
        y = self.master.winfo_y() + (self.master.winfo_height() // 2) - (550 // 2)
        about_window.geometry(f"420x550+{x}+{y}")
...
def main():
    ...
    # Parse argomenti della riga di comando
    if '-h' in sys.argv or '--help' in sys.argv:
        print("SIDPlayer C64 v4.0")
        print("\nUtilizzo: python sid_play6.py [opzioni]")
        print("\nOpzioni:")
        print("  -d, --debug    Abilita messaggi di debug a video")
        print("  -h, --help     Mostra questo aiuto")
        print("\nEsempi:")
        print("  python sid_play6.py          # Avvio normale (silenzioso)")
        print("  python sid_play6.py -d       # Avvio con debug")
        return
    
    if '-d' in sys.argv or '--debug' in sys.argv:
        DEBUG_MODE = True
        print("=" * 50)
        print("SIDPLAYER v4.0 - DEBUG MODE ENABLED")
        print("=" * 50)
    
    log_message("=== SIDPlayer Avvio ===")
    log_message(f"Versione: {VERSION}")
    if DEBUG_MODE:
        log_message("DEBUG MODE: attivo")
    ...
    if config.rawg_api_key:
        debug_print(f"✓ API Key RAWG trovata - Ricerche basate su metadata SID")
    else:
        debug_print("ℹ API Key RAWG non configurata. RAWG disabilitato.")

    if config.igdb_client_id and config.igdb_access_token:
        debug_print(f"✓ Credenziali IGDB trovate - Copertine C64 abilitate!")
        debug_print(f"  Ricerche basate su titolo SID dai metadati")
    else:
        debug_print("ℹ Credenziali IGDB non configurate.")

    if DEBUG_MODE:
        print("\n=== SIDPLAYER v4.0 ===")
        print("Supporto STIL per titoli dei subsong")
        print("Visualizzazione:")
        print("  - Titolo principale: nome del gioco (header SID)")
        print("  - Sottotitolo: titolo subsong (STIL) + autore")
        print("Copertine: cercate per titolo gioco (non subsong)")
        print("=====================\n")
