...
class RAWGGameImages:
    ...
    def clean_game_name(self, sid_filename):
        ...
        # Sostituisce underscore con spazi (tranne per casi speciali come R.I.S.K.)
        # Prima controlliamo se è un caso di lettere singole separate da underscore
        import re
        
        # Pattern per lettere singole separate da underscore (es. R_I_S_K)
        if re.match(r'^([A-Za-z]_)+[A-Za-z]$', name):
            # Trasforma R_I_S_K in R.I.S.K.
            name = name.replace('_', '.') + '.'
        else:
            # Altrimenti, sostituisce underscore con spazi
            name = name.replace('_', ' ')
        
        # Rimuove pattern comuni nei nomi dei file SID
        patterns = [
            r'\s*\(\d{4}\)\s*$',        # (1984)
            r'\s*\d{4}\s*$',            # 1984
            r'\s*\d{2}\s*$',            # 01, 02
            r'\s*(remix|version|demo|beta|alpha|unreleased)\s*$',
            # Versioni V1, V2, V3, etc.
            r'\s*[Vv]\s*\d+\s*$',       # V1, V 2
            r'\s*[Vv]\d+\s*$',          # V1, V2
            r'\s*Version\s*\d+\s*$',    # Version 1
            # Indoor/outdoor (solo per MicroProse Soccer)
            r'\s+indoor\s*$',
            r'\s+outdoor\s*$',
        ...
        # Rimuove parole comuni che non fanno parte del titolo
        # Solo se non sono l'unica parola rimasta
        words = name.split()
        if len(words) > 1:
            filtered_words = []
            for word in words:
                lower_word = word.lower()
                # Parole da rimuovere (solo se non sono l'unica parola)
                if lower_word in ['c64', 'commodore', 'sid', 'the']:
                    continue
                filtered_words.append(word)
            if filtered_words:  # Se non abbiamo rimosso tutte le parole
